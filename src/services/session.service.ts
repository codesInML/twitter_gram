...
export async function createSession (userId: string, userAgent: string): Promise<SessionOutput> {
    return await SessionDAL.create(userId, userAgent)
}

export async function findSessions ({userId, valid}: {userId: string, valid: boolean}) {
